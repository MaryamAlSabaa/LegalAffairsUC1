...
function accessFilter(user, startIndex = 1) {
  if (["Owner", "Legal Manager"].includes(user.role)) return { sql: "true", values: [] };
  if (user.role === "Legal Reviewer") return { sql: `lr.assigned_reviewer_id = $${startIndex}`, values: [user.id] };
...
export async function listRequests(user) {
  ...
  const requestsResult = await query(
    `select lr.*, c.name as category_name, d.name as department_name,
            ...
            reviewer.full_name as reviewer_name, manager.full_name as manager_name,
     ...
     left join users reviewer on reviewer.id = lr.assigned_reviewer_id
  ...
  const [documentsResult, checklistResult, suggestionsResult, commentsResult, jobsResult] = await Promise.all([
  ...
  return requestsResult.rows.map((row) => ({
    ...
    assignedReviewer: row.reviewer_name || "Not Assigned",
    ...
    reviewerComments: (commentsByRequest[row.id] || []).map((comment) => ({ authorName: comment.author_name, authorRole: comment.author_role, reviewerName: comment.author_name, text: comment.comment_text })),
...
// Legal staff can use this metadata-only register to understand the complete
// institutional workload. Documents, comments, and AI output are deliberately
// omitted; those remain available only through the assignment-aware endpoints.
export async function listRequestOverview(user) {
  const result = await query(
    `select lr.id,lr.title,lr.description,lr.priority,lr.risk_level,lr.status,
            lr.deadline,lr.submitted_at,lr.assigned_reviewer_id,
  ...
  return result.rows.map((row) => ({
    ...
    assignedReviewer: row.reviewer_name || "Not Assigned",
    assignedReviewerId: row.assigned_reviewer_id,
    isAssignedToCurrentUser: row.assigned_reviewer_id === user.id,
    priority: row.priority,
    riskLevel: row.risk_level,
    status: row.status,
    deadline: row.deadline ? new Date(row.deadline).toISOString().slice(0, 10) : "No deadline selected",
